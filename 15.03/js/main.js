...
if (postId) {
    fetchAsync(endpoint + 'posts/' + postId, showPost)
        .then(() => fetchAsync(endpoint + 'posts/' + postId + '/comments', showComments));
} else if(userId) {
    fetchAsync(endpoint + 'users/' + userId, showUser);
} else {
    fetchAsync(endpoint + 'posts', showPosts);
}
